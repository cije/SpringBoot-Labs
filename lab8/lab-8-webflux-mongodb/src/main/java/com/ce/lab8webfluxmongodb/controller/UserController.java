package com.ce.lab8webfluxmongodb.controller;

import com.ce.lab8webfluxmongodb.dao.UserRepository;
import com.ce.lab8webfluxmongodb.dataobject.UserDO;
import com.ce.lab8webfluxmongodb.dto.UserAddDTO;
import com.ce.lab8webfluxmongodb.dto.UserUpdateDTO;
import com.ce.lab8webfluxmongodb.vo.UserVO;
...
/**
 * @author c__e
 * @date Created in 2020/9/29 9:01
 */
@RestController
...
public class UserController {
    ...
    public Mono<Integer> add(UserAddDTO addDTO) {
        ...
        // 执行插入
        // 设置 USER_NULL 作为 null 的情况，否则 flatMap 不会往下走
    ...
    public Mono<Boolean> delete(@RequestParam("id") Integer id) {
        ...
        // 设置 USER_NULL 作为 null 的情况，否则 flatMap 不会往下走
        return user.defaultIfEmpty(USER_NULL)
                .flatMap((Function<UserDO, Mono<Boolean>>) userDO -> {
                    // 如果不存在该用户，则直接返回 false 失败
                    if (userDO == USER_NULL) {
                        return Mono.just(false);
                    }
