package com.ce.lab601.websocket;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.ce.lab601.handler.MessageHandler;
...
/**
 * @author c__e
 * @date Created in 2020/9/26 9:10
 */
@Controller
@ServerEndpoint("/")
public class WebsocketServerEndpoint implements InitializingBean {
    ...
    public void onClose(Session session, CloseReason closeReason) {
        logger.info("[onOpen][session({}) 连接关闭。关闭原因是({})]", session, closeReason);
        WebSocketUtil.removeSession(session);
    }

    @OnError
    public void onError(Session session, Throwable throwable) {
        logger.info("[onOpen][session({}) 发生异常({})]", session, throwable);
    }
